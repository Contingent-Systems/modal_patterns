...
\subsubsection{Actor Model} An alternative to deal with the shared mmemory concurrency complication, i.e., data races, is using actors ~\cite{actormodel} which appears as a way of sequentializing the concurrent interleaved execution with some dependency on the \emph{knowledge} about the other actors. 
...
\paragraph{Encoding Hoare Triples} With Pratt's original representation, we can encode where a Hoare triple $\{P\}C\{Q\}$ is encoded as $P\rightarrow[C](Q)$. However, this modal representation requires to be given a semantics which is in terms of an underlying model.

Conventionally, a Kripke model with its constituent must be picked. Gordon give a Kripke model for an actor-based hybrid modal logic ~\cite{gordon2019modal} in which the transition relation is indexed by a particular command and a particular actor which relates pre- and post-states of a particular actor referenced by a nominal executing the command. Gordon encodes this hybrid logic in Dafny program logic ~\cite{leino2010dafny} with the help of two state invariants and rely-guarantee assertions checking the stability of invariants. Wagner et.al. takes the similar approach for the design of their separation logic and pick up a Kripke model for reference counted memory reasoning principles.
...
%GC-Modality &      & \multicolumn{2}{c}{\multirow{2}{*}{Because why not}} & 8                                   & & ~\cite{}\\
 ...
 Ref-Count~\cite{amalreal2024}& @$_{\ell}$ P    &  $\ell_1 \mapsto v$ &  Weak  &  Allocating, Dropping and Sharing a Reference \\
  Actor~\cite{gordon2019modal}& [A(t)]P    &  $(t,x,y)$ &  Strong  &  Send Message \\\bottomrule
\end{tabular}
...
\paragraph{Actors} When dealing with reference counting APIs, we may care to specify reachability of memory nodes ($\ell \mapsto v$) in a certain context defined by a shared root address. A shared memory address $\ell$ can be the root of the graph ($@_{\ell}\; ( \ell_1 \mapsto v_1 \;\ast\; ... \; \ast \; \ell_j \mapsto v_j )$) that can be a container of memory nodes ($\ell_1 \mapsto v$) that are reachable from the root $\ell$. 
