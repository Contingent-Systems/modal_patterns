...
\paragraph{Consistency of Reclaimed Memory} A sound memory reclamation is based on the condition where there is no \emph{reachability} to the reference pointing to the allocated memory reference, and it is not \emph{shared}. Once the requirements are satisfied then the memory location can be reclaimed \emph{soundly} -- i.e., without violating any access through a reference to it.
